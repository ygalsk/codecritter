...
# Triggers a Codecritter reaction when patterns are found.
...
check_cooldown "$CODECRITTER_DIR/.last_react" "$COOLDOWN_SECS" || exit 0
...
# ── Trigger reaction ───────────────────────────────────────────────
PYTHONPATH="$PYTHONPATH" "$PYTHON" -m codecritter react --reason "$REASON" 2>/dev/null &
exit 0
