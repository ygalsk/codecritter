...
# Cooldown: 20 seconds (must match COOLDOWNS["buddy-comment"] in reactions.py).
...
check_cooldown "$JAMB_DIR/.last_comment" "$COOLDOWN_SECS" || exit 0
...
# ── Set reaction ───────────────────────────────────────────────────
PYTHONPATH="$PYTHONPATH" "$PYTHON" -m jamb buddy-comment --text "$COMMENT" 2>/dev/null &
exit 0
