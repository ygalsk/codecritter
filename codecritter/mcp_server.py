"""MCP server — expose Codecritter as tools for Claude Code."""
...
def run_server():
    """Run the Codecritter MCP server."""
    try:
        from mcp.server.fastmcp import FastMCP
    except ImportError:
        print(
            "MCP server requires the 'mcp' package. "
            'Install with: pip install "codecritter[mcp]"',
        )
        raise SystemExit(1)
    ...
    mcp = FastMCP(
        ...
        instructions=(
            ...
            "companion's voice. "
            "Check get_codecritter_status to see if reactions are muted before adding comments."
        ),
    )

    @mcp.tool()
    def get_codecritter_status() -> str:
        """Check on your companion's current status — stats, level, and personality."""
        state = persistence.load_quiet()
        return _status_text(state)

    @mcp.tool()
    def reward_codecritter(stat: str, amount: int = 3, xp: int = 10) -> str:
        """Reward your companion with stat points and XP. Use after they help with coding tasks.
        ...
        result = f"{state.name} gained +{actual_gain} {stat.upper()} and +{xp} XP!"
        if evolved:
            result += f"\n*** EVOLUTION! {state.name} evolved into {state.stage.title()}! ***"
        return result
